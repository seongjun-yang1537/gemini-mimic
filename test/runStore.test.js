...
test("RunStore.updateRun 은 동시 호출에서도 업데이트를 보존한다", async () => {
  const tempDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "run-store-"));
  const storagePath = path.join(tempDirectory, "runs.json");
  const runStore = new RunStore(storagePath);
  const createdRun = await runStore.createRun("/tmp/input.mp4");

  await Promise.all([
    runStore.updateRun(createdRun.id, { status: "phase1_done" }),
    runStore.updateRun(createdRun.id, { phase1Result: { scenario: "ok" } }),
  ]);

  const latestRun = await runStore.getRun(createdRun.id);
  assert.equal(latestRun.status, "phase1_done");
  assert.deepEqual(latestRun.phase1Result, { scenario: "ok" });
});
