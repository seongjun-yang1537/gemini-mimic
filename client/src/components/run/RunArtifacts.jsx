...
export default function RunArtifacts({ artifactItems }) {
    ...
    return (<section className="run-section">
      ...
      {isExpanded ? (artifactItems.length > 0 ? (<ul className="artifact-list">
            {artifactItems.map((artifactItem) => (<li key={artifactItem.name}>
          ...
          </ul>) : (<p className="run-placeholder">아직 생성물이 없습니다</p>)) : null}
    </section>);
}
