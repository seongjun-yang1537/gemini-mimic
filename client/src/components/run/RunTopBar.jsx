...
export default function RunTopBar({ title, status, elapsedSeconds, errorMessage, onBack, onCancel, onRetry }) {
    const showCancelButton = status === 'running';
    const showRetryButton = status === 'completed' || status === 'failed' || status === 'cancelled';
    return (<header className="run-top-bar">
      ...
      <div className="run-actions">
        {showCancelButton ? (<button type="button" className="outline-button" onClick={onCancel}>
            취소
          </button>) : null}
        {showRetryButton ? (<button type="button" className="outline-button" onClick={onRetry}>
            재실행
          </button>) : null}
      </div>
    </header>);
