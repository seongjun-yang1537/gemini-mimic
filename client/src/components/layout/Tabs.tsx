...
import { DASHBOARD_TABS, type DashboardTabKey } from '../../constants/phases';
...
export default function Tabs({ selectedTab, taskCount, onChangeTab }: TabsProps) {
  return (
    <section className="tabs-bar">
      <div className="tabs" role="tablist" aria-label="대시보드 탭">
        {DASHBOARD_TABS.map((tabItem) => (
          <button
            key={tabItem.key}
            className={`tab-button${selectedTab === tabItem.key ? ' active' : ''}`}
            type="button"
            role="tab"
            aria-selected={selectedTab === tabItem.key}
